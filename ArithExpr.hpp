...
public:
    ...
    virtual int Intevaluate(SymTab &symTab) = 0;
    virtual std::string Strevaluate(SymTab &symTab) = 0;
...
public:
    ...
    ExprNode *&right();
    virtual void print();
    virtual int Intevaluate(SymTab &symTab);
    virtual std::string Strevaluate(SymTab &symTab);
...
public:
    ...
    virtual void print();
    virtual int Intevaluate(SymTab &symTab);
    virtual std::string Strevaluate(SymTab &symTab) { return "";}
};
...
public:
    String(Token token);
    virtual void print();
    virtual int Intevaluate(SymTab &symTab) { return 0;}
    virtual std::string Strevaluate(SymTab &symTab);
};
...
public:
    Range(Token token, std::vector<ExprNode *> *iter);
    virtual void print();
    virtual int Intevaluate(SymTab  &symTab);
    virtual std::string Strevaluate(SymTab &symTab) { return "";}
    void initalizeIter(SymTab &symTab);

private:
    std::vector<ExprNode *> *_iterator;
    int _start;
    int _end;
    int _incr;
